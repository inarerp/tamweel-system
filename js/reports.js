...
// Version: 1.0.1
// ============================================================
// v1.0.1: إصلاح select('') → select('*') في loadReportsData()
// شاشة تقارير بسيطة read-only — تقرأ فقط من Financial Core.
// ============================================================
var REPORTS_STATE = { data: null, activeTab: 'company', period: 'all', from: null, to: null };
...
async function loadReportsData() {
    // ✅ v1.0.1: select('*') بدل select('') — إصلاح خطأ وظيفي حقيقي
    ...
    return { operations: operations, operationInvestors: operationInvestors, transfers: transfers, clients: clients, investors: investors, indexes: indexes };
}
...
function buildReportsIndexes(operations, operationInvestors, transfers, clients, investors) {
    ...
    var transfersByOperation = {}, transfersByInvestor = {};
    var clientOperations = {};
    operations.forEach(function(op) {
        operationsById[op.id] = op;
        if (op.client_id) { if (!clientOperations[op.client_id]) clientOperations[op.client_id] = []; clientOperations[op.client_id].push(op); }
    });
    ...
    transfers.forEach(function(t) {
        if (t.operation_id) { if (!transfersByOperation[t.operation_id]) transfersByOperation[t.operation_id] = []; transfersByOperation[t.operation_id].push(t); }
        if (t.investor_id) { if (!transfersByInvestor[t.investor_id]) transfersByInvestor[t.investor_id] = []; transfersByInvestor[t.investor_id].push(t); }
    });
    return { operationsById: operationsById, clientsById: clientsById, investorsById: investorsById, opInvestorsByOperation: opInvestorsByOperation, opInvestorsByInvestor: opInvestorsByInvestor, transfersByOperation: transfersByOperation, transfersByInvestor: transfersByInvestor, clientOperations: clientOperations };
}
...
function renderReports() {
    var container = document.getElementById('reportsContent');
    if (!container) return;
    if (!REPORTS_STATE.data) { container.innerHTML = '<div class="empty-state">لا توجد بيانات</div>'; return; }
    var tabs = [
    ...
    var html = '<div class="reports-tabs">';
    ...
    html += '</div><div class="reports-body">';
...
function reportsSwitchTab(tabId) { REPORTS_STATE.activeTab = tabId; renderReports(); }
...
function reportsCard(title, value, color) {
    return '<div class="reports-card ' + (color || '') + '">' +
        ...
        '<div class="reports-card-value">' + value + '</div></div>';
}
...
function renderOperationsProfit() {
    ...
    var html = '<div class="reports-filter">';
    ...
    if (REPORTS_STATE.period === 'custom') {
        ...
        html += '<button class="btn btn-primary btn-sm" data-action="reportsApplyCustomPeriod">تطبيق</button></div>';
    }
    ...
    if (!result.operations || !result.operations.length) { html += '<div class="empty-state">لا توجد عمليات في هذه الفترة</div>'; return html; }
...
function reportsSetPeriod(period) {
    var now = new Date();
    if (period === 'all') { REPORTS_STATE.from = null; REPORTS_STATE.to = null; }
...
function renderClientsBalances() {
    ...
    clients.forEach(function(c) {
        ...
        html += '<tr><td>' + escapeHtml(c.name || '-') + '</td><td>' + summary.totalOperations + '</td><td>' + formatMoney(summary.totalFunded) + '</td><td>' + formatMoney(summary.totalRepaid) + '</td><td>' + formatMoney(summary.balance) + '</td></tr>';
    });
    html += '</tbody></table></div>';
    return html;
}
...
function renderInvestorsBalances() {
    ...
    investors.forEach(function(i) {
        ...
        html += '<tr><td>' + escapeHtml(i.name || '-') + '</td><td>' + formatMoney(summary.totalCapital) + '</td><td>' + formatMoney(summary.fundedCapital) + '</td><td>' + formatMoney(summary.capitalPending) + '</td><td>' + formatMoney(summary.totalProfit) + '</td><td>' + formatMoney(summary.profitPaid) + '</td><td>' + formatMoney(summary.outstandingProfit) + '</td></tr>';
    });
    html += '</tbody></table></div>';
    return html;
}
...
// INIT
// ============================================================
if (typeof document !== 'undefined') { initReports(); }
// ============================================================
// END OF REPORTS.JS (v1.0.1)
// ============================================================
