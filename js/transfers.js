// ============================================================
// نظام إدارة التمويل - Transfers Module (Official Financial Ledger)
// Version: 7.0.0 (Stabilized - aligned with Financial Engine v2.1.0)
// Last Updated: 2026-08-05
// ============================================================
//
// مسؤولية هذا الملف فقط:
// - إدخال الحركة المالية / تعديلها / حذفها
// - التحقق من صحة الحركة
// - ربط الحركة بالعملية وبالطرفين (client_id / investor_id / operation_id)
// - عرض الحركة في الدفتر
//
// ممنوع في هذا الملف:
// - حساب أرصدة أو أرباح أو Coverage أو Funded/Returned
//   (كلها مسؤولية calculations.js v2.1.0 فقط)
//
// يعتمد على: core.js (runQuery, helpers, STATUS)
// يُستدعى من: app.js (أحداث)، operations.js (prefill)
// ============================================================
...
// ============================================================
// 1. TRANSFER FLOW MAP (الأنواع الستة الصحيحة)
// ============================================================
var TRANSFER_FLOW_MAP = Object.freeze({
    'company_to_client': {
        party_type: 'client', transaction_category: 'client_deposit_in', transfer_type: 'company_to_client',
        purpose_options: ['client_funding', 'additional_funding', 'settlement', 'other'], label: 'تمويل عميل'
    },
    'client_to_company': {
        party_type: 'client', transaction_category: 'client_use_out', transfer_type: 'client_to_company',
        purpose_options: ['client_repayment', 'settlement', 'other'], label: 'سداد من عميل'
    },
    'company_to_investor': {
        party_type: 'investor', transaction_category: 'investor_return_out', transfer_type: 'company_to_investor',
        purpose_options: ['capital_return', 'profit_distribution', 'settlement', 'other'], label: 'تحويل لممول'
    },
    'investor_to_company': {
        party_type: 'investor', transaction_category: 'investor_capital_in', transfer_type: 'investor_to_company',
        purpose_options: ['client_funding', 'settlement', 'other'], label: 'إيداع من ممول'
    },
    'client_to_investor': {
        party_type: 'client', transaction_category: 'client_to_investor', transfer_type: 'client_to_investor',
        purpose_options: ['settlement', 'other'], label: 'تحويل من عميل لممول'
    },
    'investor_to_client': {
        party_type: 'investor', transaction_category: 'investor_to_client', transfer_type: 'investor_to_client',
        purpose_options: ['settlement', 'other'], label: 'تحويل من ممول لعميل'
    }
});
...
function initTransfers() {
    debug('💸 بدء تهيئة transfers.js (v7.0.0)', 'info');
    if (typeof registerScreenLoader === 'function') registerScreenLoader('transfers', loadTransfers);
    debug('✅ transfers.js v7.0.0 جاهز', 'success');
}

// ============================================================
// 2. LOAD TRANSFERS (الدفتر)
// ============================================================
...
// ============================================================
// 3. RENDER LIST (عرض كل الأنواع الستة)
// ============================================================
function renderTransfersList() {
    ...
    TRANSFERS_STATE.records.forEach(function(t) {
        ...
        html += '<td>' + _displayPurpose(t.purpose) + '</td>';
...
function _getFromText(t) {
    switch (t.type) {
        ...
        case 'client_to_company': case 'client_to_investor': return _clientLink(t);
        case 'investor_to_company': case 'investor_to_client': return _investorLink(t);
        default: return '-';
    }
}
function _getToText(t) {
    switch (t.type) {
        ...
        case 'company_to_client': case 'investor_to_client': return _clientLink(t);
        case 'company_to_investor': case 'client_to_investor': return _investorLink(t);
        default: return '-';
    }
}
...
function _buildAllPartiesOptions() {
    ...
    if (clients.length > 0) {
        ...
        clients.forEach(function(c) {
            if (!c.is_archived) options += '<option value="client:' + c.id + '" data-type="client">👤 ' + escapeHtml(c.name) + '</option>';
        });
        options += '</optgroup>';
    }
    ...
    if (investors.length > 0) {
        ...
        investors.forEach(function(inv) {
            if (!inv.is_archived) options += '<option value="investor:' + inv.id + '" data-type="investor">💼 ' + escapeHtml(inv.name) + '</option>';
        });
        options += '</optgroup>';
    }
    return options;
...
function updateTransferFields() {
    ...
    // إخفاء حقول الكيانات القديمة (إن وجدت في HTML)
    ...
    if (from.type && to.type) {
        var flow = TRANSFER_FLOW_MAP[_determineTransferType(from.type, to.type)];
        if (flow) {
            _updatePurposeField(from.type, to.type);
            _updateTransferSummary(from.type, to.type);
...
function _updatePurposeField(fromType, toType) {
    ...
    if (!fromType || !toType) { purposeRow.style.display = 'none'; return; }
    var flow = TRANSFER_FLOW_MAP[_determineTransferType(fromType, toType)];
    if (!flow) { purposeRow.style.display = 'none'; return; }
...
function _updateTransferSummary(fromType, toType) {
    var summaryEl = document.getElementById('transferSummary');
    if (!summaryEl) return;
    if (!fromType || !toType) { summaryEl.style.display = 'none'; return; }
    var flow = TRANSFER_FLOW_MAP[_determineTransferType(fromType, toType)];
...
// ============================================================
// 7. POPULATE FORM (EDIT) — ✅ يحفظ transferId فيعدّل نفس السجل
// ============================================================
function populateTransferForm(transfer, title) {
    var titleEl = document.getElementById('transferModalTitle');
    if (titleEl) titleEl.textContent = title;

    // ✅ إصلاح جوهري: تثبيت الـ id حتى لا ينشئ التعديل سجلاً جديداً
...
function resetTransferForm() {
    ...
    var purposeRow = document.getElementById('transferPurposeRow'); if (purposeRow) purposeRow.style.display = 'none';
    var summary = document.getElementById('transferSummary'); if (summary) summary.style.display = 'none';
}

// ============================================================
// 8. PREFILL (يستدعى من operations.js)
// ============================================================
function _applyPrefill(prefill) {
    ...
    if (prefill.fromType && prefill.fromEntity) { if (fromEl) fromEl.value = prefill.fromType + ':' + prefill.fromEntity; }
    else if (prefill.fromType === 'company') { if (fromEl) fromEl.value = 'company:company'; }
    if (prefill.toType && prefill.toEntity) { if (toEl) toEl.value = prefill.toType + ':' + prefill.toEntity; }
    else if (prefill.toType === 'company') { if (toEl) toEl.value = 'company:company'; }
...
function collectTransferFormData() {
    ...
    return {
        ...
        clientId: clientId, investorId: investorId,
...
async function validateTransferForm(formData) {
    if (!formData.fromType || !formData.toType) { showToast('❌ يجب اختيار من وإلى', 'error'); return false; }
    if (formData.fromType !== 'company' && !formData.fromId) { showToast('❌ يجب اختيار الطرف المصدر', 'error'); return false; }
    if (formData.toType !== 'company' && !formData.toId) { showToast('❌ يجب اختيار الطرف المستلم', 'error'); return false; }
    if (formData.fromType === formData.toType && formData.fromId === formData.toId) { showToast('❌ لا يمكن التحويل من الطرف إلى نفسه', 'error'); return false; }
    if (formData.fromType === 'company' && formData.toType === 'company') { showToast('❌ لا يمكن التحويل من الشركة إلى نفسها', 'error'); return false; }

    var flow = TRANSFER_FLOW_MAP[_determineTransferType(formData.fromType, formData.toType)];
...
// ============================================================
// 10. SAVE TRANSFER (إدراج أو تحديث نفس السجل)
// ============================================================
...
// ============================================================
// 11. DELETE TRANSFER (السجل المطلوب فقط)
// ============================================================
...
// ============================================================
// 13. DISPLAY TEXT HELPERS (عرض فقط — ليست حسابات)
// ============================================================
...
function getPurposeText(purpose) { return PURPOSE_TEXT_AR[purpose] || purpose || '-'; }
function _displayPurpose(purpose) { return getPurposeText(purpose); }
