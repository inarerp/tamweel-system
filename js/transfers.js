...
// Version: 6.1.0 (Fixed: edit-same-record + summary + edit aliases)
...
// ✅✅✅ إصلاح: كان بيرجع fromType+'to'+toType (بدون شرطات) فملخص/Gرض مش ظهروا
function _determineTransferType(fromType, toType) {
return fromType + '_to_' + toType;
}

// ============================================================
// 4. OPEN MODAL (مع prefill)
// ============================================================
...
var options = '';
flow.purpose_options.forEach(function(p) { options += '<option value="' + p + '">' + (PURPOSE_TEXT_AR[p] || p) + '</option>'; });
...
// ============================================================
// 7. POPULATE FORM (EDIT)  ✅ إصلاح: بيحط transferId دلوقتي
// ============================================================
function populateTransferForm(transfer, title) {
var titleEl = document.getElementById('transferModalTitle');
if (titleEl) titleEl.textContent = title;
// ✅✅✅ الإصلاح الجوهري: حفظ الـ id عشان التعديل يعدّل نفس السجل
...
setTimeout(function() { if (transfer.purpose) _setTransVal('transferPurpose', transfer.purpose); }, 100);
}
function resetTransferForm() {
['transferId', 'transferFromType', 'transferToType', 'transferAmount',
'transferOperation', 'transferNotes', 'transferTransactionCategory', 'transferPurpose'].forEach(function(id) { _setTransVal(id, ''); });
...
fromType: from.type, toType: to.type, clientId: clientId, investorId: investorId,
...
if (formData.fromType === formData.toType) { showToast('❌ لا يمكن التحويل من وإلى نفس النوع', 'error'); return false; }
...
var data = {
type: flow.transfer_type, purpose: selectedPurpose,
operation_id: formData.operationId || null,
client_id: formData.clientId || null, investor_id: formData.investorId || null,
amount: parseFloat(formData.amount), transfer_date: formData.transferDate,
notes: formData.notes || null, party_type: flow.party_type,
...
var warningMsg;
if (WORKFLOW_TRANSFER_PURPOSES.indexOf(transfer.purpose) !== -1) warningMsg = '⚠️ تحذير: هذا التحويل مرتبط بسير العمل وحذفه سيؤثر على الأرصدة.\n\nهل أنت متأكد؟';
else if (transfer.operation_id) warningMsg = '⚠️ هذا التحويل مرتبط بعملية. حذفه سيؤثر على حالة التمويل.\n\nهل أنت متأكد؟';
else warningMsg = 'هل أنت متأكد من حذف هذا التحويل؟';
...
} catch (err) { showToast(handleSupabaseError(err, 'حذف التحويل'), 'error'); }
finally { hideLoading(); }
}
...
// ============================================================
// 14. ✅ EDIT ALIASES (بتصلح زرار تعديل العميل/الممول من غير لمس ملفات تانية)
// ============================================================
