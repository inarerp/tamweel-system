...
async function loadOpTransfersTab(operationId) {
    ...
    var result = await runQuery(function() { 
        return APP.supabase.from('transfers').select('*').eq('operation_id', operationId).order('transfer_date', {ascending: false}); 
...
async function loadOpTimelineTab(operationId) {
    ...
    var result = await runQuery(function() { 
        return APP.supabase.from('activity_logs').select('*').eq('entity_id', operationId).eq('entity_type', 'operation').order('created_at', {ascending: false}).limit(50); 
...
async function workflowAction(action) {
    ...
    switch(action) {
        case 'activate': confirmMsg = 'هل تريد تفعيل هذه العملية؟'; newStatus = 'active'; break;
        case 'complete': confirmMsg = 'هل تريد إنهاء هذه العملية؟ سيتم قفلها.'; newStatus = 'completed'; break;
        case 'unlock': confirmMsg = 'هل تريد فتح قفل العملية للتعديل؟'; break;
        case 'clientRepayment': openWorkflowTransfer('client_repayment'); return;
        case 'profitDistribution': openWorkflowTransfer('profit_distribution'); return;
        case 'capitalReturn': openWorkflowTransfer('capital_return'); return;
    }
    ...
    try {
        if (newStatus) {
            await runQuery(function() { 
                return APP.supabase.from('operations').update({ status: newStatus, is_locked: newStatus === 'completed' }).eq('id', opId); 
            }, { context: 'workflowAction', throwError: true });
        } else if (action === 'unlock') {
            await runQuery(function() { 
                return APP.supabase.from('operations').update({ is_locked: false }).eq('id', opId); 
            }, { context: 'workflowAction-unlock', throwError: true });
        }
        
        if (typeof window.logActivityToDB === 'function') {
            window.logActivityToDB('Workflow: ' + action, 'operation', opId, null, null, 'Action: ' + action, 'workflow');
        }
...
function searchOperations(term) { OPERATIONS_STATE.search = term; loadOperations(); }
function filterOperations(status) { OPERATIONS_STATE.filter = status; loadOperations(); }
